...
export async function getRegionFicha(id: number) {
  ...
  const [nacion] = region.nacionId
    ? await db.select({ id: s.naciones.id, nombre: s.naciones.nombre }).from(s.naciones).where(eq(s.naciones.id, region.nacionId)).limit(1)
    : [undefined];

  const locaciones = await db
    .select({ id: s.locaciones.id, nombre: s.locaciones.nombre, tipo: s.locaciones.tipo, imagenUrl: s.locaciones.imagenUrl, subtitulo: s.locaciones.subtitulo })
    .from(s.locaciones)
...
export async function getLocacionFicha(id: number) {
  ...
  const [region] = locacion.regionId
    ? await db.select({ id: s.regiones.id, nombre: s.regiones.nombre }).from(s.regiones).where(eq(s.regiones.id, locacion.regionId)).limit(1)
    : [undefined];
  const [nacion] = locacion.nacionId
    ? await db.select({ id: s.naciones.id, nombre: s.naciones.nombre }).from(s.naciones).where(eq(s.naciones.id, locacion.nacionId)).limit(1)
    : [undefined];
