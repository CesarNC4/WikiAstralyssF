...
import { cambiarEstadoPersonaje, moverAPapelera } from "@/lib/actions/personajes";
...
export function PersonajesTable({
  items,
  total,
  page,
  pageSize,
  ...
  const navigate = (params: Record<string, string | number>) => {
    ...
    router.push(`/admin/personajes${sp.toString() ? `?${sp}` : ""}`);
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <select
          value={estado}
          onChange={(e) => navigate({ estado: e.target.value, page: 1 })}
        ...
        <button type="submit" className="rounded-lg border border-border-glow px-3 py-2 text-sm text-fg-secondary hover:text-fg">
          Buscar
        </button>
        <span className="ml-auto text-xs text-fg-muted">{total} personajes</span>
      </form>
      ...
      <div className="overflow-hidden rounded-2xl border border-border-base">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((p) => (
              <tr key={p.id} className="border-t border-border-base hover:bg-surface/30">
                <td className="px-3 py-2">
                  <div className="flex items-center gap-2">
                    <div className="relative h-9 w-9 shrink-0 overflow-hidden rounded-lg bg-deep">
                      {p.imagenUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={cldOptimize(p.imagenUrl) ?? ""} alt={p.nombre} className="h-full w-full object-cover" />
                      ) : (
                        <span className="grid h-full w-full place-items-center text-xs text-border-glow">✦</span>
                      )}
                    </div>
                    <div className="min-w-0">
                      <Link href={`/admin/personajes/${p.id}/editar`} className="block truncate text-fg hover:text-primary-glow">
                        {[p.nombre, p.surname].filter(Boolean).join(" ")}
                      </Link>
                      {p.titulo && <span className="block truncate text-xs text-fg-muted">{p.titulo}</span>}
                    </div>
                  </div>
                </td>
                <td className="px-3 py-2"><EstadoBadge estado={p.estado} /></td>
                <td className="hidden px-3 py-2 text-xs text-fg-muted sm:table-cell">{relativo(p.actualizadoEn)}</td>
                <td className="px-3 py-2">
                  <div className="flex items-center justify-end gap-1 text-xs">
                    <Link href={`/admin/personajes/${p.id}/editar`} className="rounded border border-border-base px-2 py-1 text-fg-secondary hover:text-fg">Editar</Link>
                    <Link href={`/admin/preview/personaje/${p.id}`} target="_blank" className="rounded border border-border-base px-2 py-1 text-fg-secondary hover:text-fg">Preview</Link>
                    {p.estado !== "publicado" ? (
                      <button disabled={pending} onClick={() => accion(() => cambiarEstadoPersonaje(p.id, "publicado"), "Publicado.")} className="rounded border border-success/40 px-2 py-1 text-success hover:bg-success/10">Publicar</button>
                    ) : (
                      <button disabled={pending} onClick={() => accion(() => cambiarEstadoPersonaje(p.id, "oculto"), "Oculto.")} className="rounded border border-warning/40 px-2 py-1 text-warning hover:bg-warning/10">Ocultar</button>
                    )}
                    <button
                      disabled={pending}
                      onClick={() => { if (confirm("¿Mover a la papelera?")) accion(() => moverAPapelera(p.id), "Movido a la papelera."); }}
            ...
            {items.length === 0 && (
              <tr><td colSpan={4} className="px-3 py-8 text-center text-fg-muted">No hay personajes con estos filtros.</td></tr>
            )}
          </tbody>
        </table>
      </div>
